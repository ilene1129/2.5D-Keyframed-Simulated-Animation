...
def output_chain(chain, frame_num, stroke_num):
    ...
    with open("/Users/ilenee/Documents/2020-2021/Thesis/3_16/bunny/output_frame"+str(frame_num)+"stroke"+str(stroke_num)+".csv", "w") as f:
        writer = csv.writer(f)
...
for ind in range(15, 23):
    ...
    if ind < 15 or ind > 18:
        ...
        # if len(chains) > 0:
        #     world.CreateDistanceJoint(bodyA=chains[ind - 1 - 15][-1], bodyB=chain[0], anchorA=chains[ind - 1 - 15][-1].worldCenter, anchorB=chain[0].worldCenter, collideConnected = True)
    chains.append(chain)

# world.CreateDistanceJoint(bodyA=chains[5][-1], bodyB=dynamic_chains[0][0], anchorA=chains[5][-1].worldCenter, anchorB=dynamic_chains[0][0].worldCenter, collideConnected=True)
# world.CreateDistanceJoint(bodyA=chains[7][-1], bodyB=dynamic_chains[-1][-1], anchorA=chains[7][-1].worldCenter, anchorB=dynamic_chains[-1][-1].worldCenter, collideConnected=True)
...
env = [border_body1, border_body2, border_body3, border_body4]
# env = [env_body]
# env = []
...
PERIOD = 15
period_counter = 0
rand = [random.random() / 2, random.random(), random.random()]
...
while running:
    ...
    if period_counter == PERIOD:
        PERIOD = math.ceil(random.random()*5+12.5)
        period_counter = 0
        rand = [random.random() / 2, random.random()*2, random.random()*2]

    for ind in range(len(dynamic_chains[0])):
        body = dynamic_chains[0][ind]
        # if int(ind * 4 / len(dynamic_chains[1])) % 2 == 1:
        body.ApplyForce(rand[0] * force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2  * math.pow(ind, 2) / len(dynamic_chains[0]), body.position, True)

    for ind in range(len(dynamic_chains[1])):
        body = dynamic_chains[1][ind]
        # if int(ind * 4 / len(dynamic_chains[1])) % 2 == 1:
        body.ApplyForce(rand[0] * force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2  * math.pow((len(dynamic_chains[1]) - ind), 2) / len(dynamic_chains[1]), body.position, True)

    for ind in range(len(dynamic_chains[2])):
        body = dynamic_chains[2][ind]
        # if int(ind * 4 / len(dynamic_chains[1])) % 2 == 1:
        body.ApplyForce(rand[1] * force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2  * math.pow(min(len(dynamic_chains[2]) - ind, ind), 2) / len(dynamic_chains[2]), body.position, True)

    for ind in range(len(dynamic_chains[3])):
        body = dynamic_chains[3][ind]
        # if int(ind * 4 / len(dynamic_chains[1])) % 2 == 1:
        body.ApplyForce(rand[2] * force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2  * math.pow(min(len(dynamic_chains[3]) - ind, ind), 2) / len(dynamic_chains[3]), body.position, True)

    # for ind in range(len(chains[0])):
    #     body = chains[0][ind]
    #     body.ApplyForce(force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2 * abs(frame) / PERIOD * abs(ind) / len(chain), body.position, True)
    
    # for ind in range(len(chains[1])):
    #     body = chains[1][ind]
    #     body.ApplyForce(force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2 * abs(frame) / PERIOD * abs(ind) / len(chain), body.position, True)

    # for ind in range(len(chains[2])):
    #     body = chains[2][ind]
    #     body.ApplyForce(force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2 * abs(frame) / PERIOD * (len(chain) - abs(ind - len(chain) * 0.5)) / len(chain), body.position, True)

    # for ind in range(len(chains[3])):
    #     body = chains[3][ind]
    #     body.ApplyForce(force_vec * force_magnitude * (math.sin(frame * 2 * math.pi / PERIOD) + 1) / 2 * abs(frame) / PERIOD * (len(chain) - abs(ind - len(chain) * 0.5)) / len(chain), body.position, True)
    ...
    for ind in range(len(chains)):
        update_chain(chains[ind], force_magnitude, frame, ind + 15)
